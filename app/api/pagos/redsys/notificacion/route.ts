...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!notification.successful) {
      await DB.batch([
        DB.prepare("INSERT OR IGNORE INTO payment_events (payment_id, provider, event_key, event_type, payload_hash) VALUES (?, 'redsys', ?, 'payment_failed', ?)").bind(payment.id, eventKey, notification.payloadHash),
        DB.prepare("UPDATE payments SET status = 'failed', active_scope_key = NULL, response_code = ?, callback_hash = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND status != 'confirmed'").bind(notification.responseCode, notification.payloadHash, payment.id),
      ]);
    ...
    const statements = [
      DB.prepare("INSERT OR IGNORE INTO payment_events (payment_id, provider, event_key, event_type, payload_hash) VALUES (?, 'redsys', ?, 'payment_confirmed', ?)").bind(payment.id, eventKey, notification.payloadHash),
      DB.prepare("UPDATE payments SET status = 'confirmed', active_scope_key = NULL, provider_transaction_id = ?, response_code = ?, callback_hash = ?, validated_at = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND status != 'confirmed'").bind(notification.authorizationCode, notification.responseCode, notification.payloadHash, validatedAt, payment.id),
      DB.prepare("INSERT OR IGNORE INTO invoices (payment_id, status) VALUES (?, 'not_requested')").bind(payment.id),
    ];
    if (payment.participant_id) statements.push(DB.prepare("UPDATE participants SET payment_status = 'paid', payment_method = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(payment.method, payment.participant_id));
    const results = await DB.batch(statements);
    const transitioned = Number((results[1] as { meta?: { changes?: number } })?.meta?.changes || 0) > 0;

    const totals = await DB.prepare(`SELECT coalesce((SELECT sum(oi.unit_price_cents + oi.extras_cents) FROM order_items oi INNER JOIN participants p ON p.id = oi.participant_id WHERE p.group_id = ?), 0) AS expected, coalesce((SELECT sum(amount_cents) FROM payments WHERE group_id = ? AND status = 'confirmed'), 0) AS confirmed`).bind(payment.group_id, payment.group_id).first<{ expected: number; confirmed: number }>();
    if (Number(totals?.expected || 0) > 0 && Number(totals?.expected) === Number(totals?.confirmed)) {
      await DB.batch([
        DB.prepare("UPDATE group_orders SET payment_status = 'complete', production_status = 'queued', updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(payment.group_id),
