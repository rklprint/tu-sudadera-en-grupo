  ...
  return <main className="homepage-flow">
    ...
    <section className="customizer-section" id="personalizador">
      ...
      <div className="customizer-shell">
        <div className="customizer-progress"><span style={{width:`${progress}%`}} /><div>{chapters.map((item,index)=><button key={item} className={openStep===index?"active":openStep>index?"done":""} aria-current={openStep===index ? "step" : undefined} onClick={()=>setOpenStep(index)}><i>{openStep>index?"✓":String(index+1).padStart(2,"0")}</i>{item}</button>)}</div></div>
...
function FlagMark({type}:{type:string}){return <span className={`flag-mark ${type}`} data-flag={type} aria-label={type === "spain" ? "Bandera de España" : "Marca de manga"} aria-hidden="true" />}
...
function Hoodie({color,printColor,text,side,designStyle="default",frontType="name",frontText="PROMO",frontLogo="",sleeveFlag="",sleeveTechnique="print"}:{color:string;printColor:string;text:string;side:Side;designStyle?:string;frontType?:string;frontText?:string;frontLogo?:string;sleeveFlag?:string;sleeveTechnique?:string}){const visual=CORE_COLORS.find(item=>item.value===color);const image=side==="front"?visual?.frontImage:visual?.backImage;return <div className="hoodie hoodie-real" style={{"--hoodie":color,"--print":printColor} as React.CSSProperties}>{image&&<img className="hoodie-real-asset" src={image} alt={visual?.name+" "+(side==="front"?"frontal":"trasera")} width={2000} height={2000} loading={side==="front"?"eager":"lazy"} decoding="async" />}{sleeveFlag&&<div className={`hoodie-sleeve-design ${sleeveTechnique}`}><span className={`hoodie-flag ${sleeveFlag}`} data-flag={sleeveFlag} aria-label={sleeveFlag === "spain" ? "Bandera de España" : "Marca de manga"} aria-hidden="true" /></div>}<div className="hoodie-body">{side==="back"?<div className={`hoodie-design back design-${designStyle}`}>{text.split("\n").map((line,index)=><span key={line+"-"+index}>{line}</span>)}</div>:<div className={`front-personalization ${frontType}`}>{frontType==="logo"?(frontLogo?<span className="uploaded-logo" role="img" aria-label="Logo subido" style={{backgroundImage:`url(${frontLogo})`}}/>:<><b>◇</b><span>LOGO</span></>):<span>{frontText||(frontType==="coordinates"?"COORDENADAS":"NOMBRE")}</span>}</div>}</div></div>}
