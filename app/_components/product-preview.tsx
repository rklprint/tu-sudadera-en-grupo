...
export function ProductPreview({ product, color, side, design, summary, overlay, onLoad }: {
  ...
  const backLines = overlay.backLines.map(line => line.trim()).filter(Boolean).slice(0, 3);
  return <div className="product-preview-frame" style={printStyle}>
    {src && failed !== src ? <>
      ...
      {side === 'front' && <div className={`product-preview-print-mark front ${overlay.frontType}`} aria-hidden="true">{overlay.frontType === 'logo' ? 'LOGO' : overlay.frontText || (overlay.frontType === 'coordinates' ? 'COORDENADAS' : 'NOMBRE')}</div>}
      {side === 'back' && !visibleDesign && backLines.length > 0 && <div className="product-preview-print-mark back" aria-hidden="true">{backLines.map((line, index) => <span key={`${line}-${index}`}>{line}</span>)}</div>}
