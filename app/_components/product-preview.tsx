...
import { isDefinitiveHoodieMockup } from '@/lib/hoodie-mockups';
...
export function ProductPreview({ product, color, side, design, summary, overlay, sleeve, onLoad, onAssetError }: {
  ...
  const definitiveHoodie = product.slug === 'sudadera-gildan-18500' && isDefinitiveHoodieMockup(src, side);
  ...
  return <div className="product-preview-frame" data-calibrated={Boolean(placement)} data-hoodie-mockup={definitiveHoodie || undefined} style={printStyle}>
    {src && failed !== src ? <>
      <Image key={src} unoptimized src={src} alt={`${product.model} · ${color.name} · ${side === 'front' ? 'Delante' : 'Espalda'}`} width={definitiveHoodie ? 1139 : 2000} height={definitiveHoodie ? 1381 : 2000} loading="eager" sizes="(max-width: 700px) 94vw, 620px" onLoad={onLoad} onError={() => { setFailed(src); onAssetError(src); onLoad(); }} />
      {visibleDesign && <div className="product-preview-artwork" style={{ position: 'absolute', left: `${design.position.x}%`, top: `${design.position.y}%`, width: `${design.size.width}%`, height: `${design.size.height}%` }}>
        {design.preview?.nameField && <svg className="product-preview-back-name" viewBox="0 0 180 42" role="img" aria-label={`Nombre sobre el diseño: ${backName}`}><text x="90" y="32" textAnchor="middle" fontSize="33" fontWeight="800" fill={overlay.printColor} textLength={backName.length > 11 ? 176 : undefined} lengthAdjust="spacingAndGlyphs">{backName}</text></svg>}
